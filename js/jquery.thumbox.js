...
(function($,doc){
    $.fn.thumbox = function(settings) {
        ...
        showDock = function(objs, params, autoopen){
            ...
            _isSingleImg     = false;
            tTop = (getPlParam('dockPosition') == 'top') ? ($(document).scrollTop()) : ($(window).height()+$(document).scrollTop()-$('#thumbox-dock').height()-10);
            $('#thumbox-dock').css({
                'top':tTop,
                'width':wmax+40,
        ...
        var showImage = function(cord, urlImg, label, imgindex){
            ...
            objImagePreloader.onload  = function(){
                ...
                $(img).css(cord).show().animate({
                    ...
                    left:($(window).width()/2)-((imgWidth*450)/imgHeight/2),
                    top:($(window).height()/2)-(458/2)+$(document).scrollTop(),
                    width:(imgWidth*450)/imgHeight,
                    height:450
                    ...
                    $('#thumbox-border').hide().css({
                        /*width:imgWidth+12,
                        height:imgHeight+12,*/						
						width:(imgWidth*450)/imgHeight+8,
						height:450+8,
                        left:pos.left-4,
                        top:pos.top-4
                        ...
                        $('#thumbox-close').bind('click', function(){
                            ...
                            left:pos.left-46+(imgWidth*450)/imgHeight,
                            top:pos.top+6
                        });
                    });
                    //botoes de navegacao
                    ...
                    if((imgindex != _qtdImgs) && (imgindex != undefined)){
                        ...
                        $('#thumbox-buttonNext').bind('click', function(){
                            if(isLocked()) return;
                          //  openNext()   
           						 openImageFromDock(parseInt(_currentImage)+1)
                        })
                    }
                    /*topBtn = pos.top+imgHeight/2-33;*/
					topBtn = pos.top+458/2 - 20;
                    ...
                    $('#thumbox-buttonNext').css({
                        top:topBtn, 
                        /*left:pos.left+(imgWidth*450)/imgHeight+25*/
						right:5
                        });
                    if(!$.browser.msie){
                    ...
                    if(getPlParam('showLabel') && (label != '') && (label != undefined)){
						...
						topPosLabel = (getPlParam('labelPosition') == 'bottom') ? pos.top+450 : pos.top-25-$('#thumbox-label').height();
                        $('#thumbox-label').css({
                            top:topPosLabel,
                            /*left:pos.left+(imgWidth/2)-($('#thumbox-label').width()/2)*/
							left:pos.left+((imgWidth*430)/imgHeight)/2-($('#thumbox-label').width()/2)
                        });
                        if(!$.browser.msie){
